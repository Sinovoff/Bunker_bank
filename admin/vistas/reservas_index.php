	...
	<table id="example" class="table table-bordered table-striped">
		...
		<thead>
			<tr>
				...
				<th class="col-md-5 <?if ($orden=="idencriptado") echo $claseflecha?>">
					<a href="<?echo $rutaorden?>&orden=idencriptado"			title="Referencia">Referencia</a></th>
				...
				<th class="<?if ($orden=="sala_id") echo $claseflecha?>">
					<a href="<?echo $rutaorden?>&orden=sala_id"					title="Sala">Sala</a></th>
				<th class="<?if ($orden=="total") echo $claseflecha?>">
					<a href="<?echo $rutaorden?>&orden=total"					title="Precio">Precio</a></th>
				<th class="<?if ($orden=="cupondescuento") echo $claseflecha?>">
					<a href="<?echo $rutaorden?>&orden=cupondescuento"			title="Cupón">Cupón</a></th>
				...
				<th class="col-md-1 <?if ($orden=="pagado") echo $claseflecha?>">
					<a href="<?echo $rutaorden?>&orden=pagado"					title="Pagado">Pagado</a></th>
		...
		<tbody>
			<?foreach($arrayRegistros as $registro){
				...
				<tr>
					...
					<td><?echo utf8_encode(stripslashes($registro->idencriptado))?></td>
					<td class="text-center"><?if ($registro->fechareserva!="") echo $registro->fechareserva->format('d-m-Y H:i')?></td>
					<td><?echo utf8_encode(stripslashes($registro->sala->nombre))?></td>
					<td class="text-right"><?echo number_format($registro->total, 2, ',', '')?></td>
					<td class="text-center"><?echo utf8_encode(stripslashes($registro->cupondescuento))?></td>
					<td><?echo utf8_encode(stripslashes($registro->nombre))?></td>
					<td><?echo utf8_encode(stripslashes($registro->email))?></td>
					<td class="text-center"><a href="#" class="cambiarcampo" id="pagado_<?echo $registro->id?>"><?echo campo_Activo($registro->pagado) ?></a></td>
