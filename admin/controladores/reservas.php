...
class Reservas{
	...
	function index(){
		...
		//*************** Campos de busqueda ********
		$arraycampos=array("idencriptado", "nombre", "email");
		...
		$opciones = array('conditions' => "activo='1'");	
		$Registrosactivos  = Count($Modelo::find('all',$opciones));

		/*$options=	"SELECT usuarios.*, provincias.provincia FROM usuarios ".
					"LEFT JOIN provincias ON usuarios.provincia_id=provincias.id ".
					"WHERE 1 AND ".$filtro." ORDER BY ".$orden." ".$tipoorden." LIMIT ".$inicio.",".$tamano_pagina;
		$options2 = "SELECT usuarios.*, provincias.provincia FROM usuarios ".
					"LEFT JOIN provincias ON usuarios.provincia_id=provincias.id ".
					"WHERE 1 AND ".$filtro;
	
		$arrayRegistros =  $Modelo::find_by_sql($options);
		$numeroRegistros  = Count($Modelo::find_by_sql($options2));*/
	...
	function guardar($id){
		...
		if ($tiporegistro==$this->opcion){
			...
			if($id>0){
				...
				$idencriptado=generar_referencia("Reserva",12);

				$registro  = new $Modelo();
				
				$registro->idencriptado	= $idencriptado;
			}

			
			$registro->sala_id		= utf8_decode(addslashes($_POST['sala_id']));
			...
			$registro->total		= utf8_decode(addslashes($_POST['total']));
			$registro->pagado		= addslashes($_POST['pagado']);
